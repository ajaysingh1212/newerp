    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.login-logs.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('use') ? 'is-invalid' : '' }}" name="use_id" id="use_id" required>
                    @foreach($uses as $id => $entry)
                        <option value="{{ $id }}" {{ old('use_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('use'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('logged_in_at') ? 'is-invalid' : '' }}" type="text" name="logged_in_at" id="logged_in_at" value="{{ old('logged_in_at', '') }}">
