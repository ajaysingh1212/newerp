...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.imeiMaster.title_singular') }}
    </div>

    <div class="card-body">
        @include('watermark')
        <form method="POST" action="{{ route("admin.imei-masters.store") }}" enctype="multipart/form-data" class="row">
            @csrf
            <div class="form-group col-md-6">
                ...
                <select class="form-control select2 {{ $errors->has('imei_model') ? 'is-invalid' : '' }}" name="imei_model_id" id="imei_model_id" required>
                    @foreach($imei_models as $id => $entry)
                        <option value="{{ $id }}" {{ old('imei_model_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group col-md-6">
                ...
                <input class="form-control {{ $errors->has('imei_number') ? 'is-invalid' : '' }}" type="text" name="imei_number" id="imei_number" value="{{ old('imei_number', '') }}" required>
            ...
            <div class="form-group col-md-6">
                ...
                <select class="form-control {{ $errors->has('product_status') ? 'is-invalid' : '' }}" name="product_status" id="product_status">
                    ...
                    @foreach(App\Models\ImeiMaster::PRODUCT_STATUS_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('product_status', 'not-used') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
