...
<style>
    ...
    .me-steps li:not(:last-child)::after {
        content: ''; position: absolute; top: 18px; left: 60%; width: 80%; height: 2px; background: rgba(255,255,255,.2); z-index: -1;
    }
    ...
    .me-btn-nav { background: linear-gradient(135deg, #7C3AED, #6366F1); border: none; color: #fff; border-radius: 8px; padding: 10px 24px; transition: .2s; }
    .me-btn-nav:hover { transform: translateY(-1px); box-shadow: 0 6px 16px rgba(124,58,237,.35); color: #fff; }
    .me-btn-outline { border: 1px solid #C4B5FD; color: #6D28D9; background: #fff; border-radius: 8px; padding: 10px 24px; }
    @media (max-width: 576px) { .me-steps li span.label-text { display: none; } .me-wizard-header { padding: 16px; } }
</style>

<div class="card me-wizard-card animate__animated animate__fadeIn">
    <div class="me-wizard-header">
        <h4 class="mb-0"><i class="fas fa-user-tie mr-2"></i>Add New Party</h4>
        <small>Manual Entry &raquo; Party &raquo; Naya party banayein</small>
        <ul class="me-steps">
            <li class="active" data-step="1"><div class="circle">1</div><span class="label-text">Basic Info</span></li>
            <li data-step="2"><div class="circle">2</div><span class="label-text">Location</span></li>
            <li data-step="3"><div class="circle">3</div><span class="label-text">GST / PAN</span></li>
            <li data-step="4"><div class="circle">4</div><span class="label-text">Account</span></li>
        </ul>
    </div>
    ...
    <div class="card-body p-4">
        ...
        <form action="{{ route('admin.manual-parties.store') }}" method="POST" id="partyForm">
            @csrf
            ...
            {{-- STEP 2: Location (JSON-based cascading dropdowns) --}}
            ...
            <div class="d-flex justify-content-between mt-4">
                ...
                <div class="ml-auto">
                    ...
                    <button type="submit" class="me-btn-nav" id="submitBtn" style="display:none;"><i class="fas fa-check mr-1"></i> Save Party</button>
                </div>
            </div>
        </form>
    </div>
...
(function() {
    ...
    // ---------- JSON-based Cascading State -> District -> City ----------
    ...
    districtSelect.addEventListener('change', function () {
        ...
        if (districtObj && districtObj.cities) {
            districtObj.cities.forEach(c => {
                ...
                citySelect.innerHTML += `<option value="${cityName}" data-pincode="${c.pincode ?? ''}">${cityName}</option>`;
            });
    ...
    citySelect.addEventListener('change', function () {
        ...
        if (pin && !pincodeInput.value) pincodeInput.value = pin;
    });
})();
</script>
@endsection
