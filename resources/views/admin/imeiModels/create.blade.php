...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.imeiModel.title_singular') }}
    </div>

    <div class="card-body">
        @include('watermark')
        <form method="POST" action="{{ route("admin.imei-models.store") }}" enctype="multipart/form-data" >
            @csrf
            <div class="card px-3">
                <h4 class= "text-center mt-2 py-2 bg-1"> Create Imei Models</h4>
         <div class="row">
            <div class="form-group col-md-6">
                <label class="required" for="imei_model_number">{{ trans('cruds.imeiModel.fields.imei_model_number') }}</label>
                <input class="form-control {{ $errors->has('imei_model_number') ? 'is-invalid' : '' }}" type="text" name="imei_model_number" id="imei_model_number" value="{{ old('imei_model_number', '') }}" required>
