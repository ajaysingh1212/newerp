...
<div class="card">
    ...
    <div class="card-body">
        ...
        <form method="POST" action="{{ route("admin.districts.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="card px-3">
                <h4 class= "text-center mt-2 py-2 bg-1"> Create District</h4>
         <div class="row">
            <div class="form-group col-lg-4">
                <label class="required" for="districts">{{ trans('cruds.district.fields.districts') }}</label>
                <input class="form-control {{ $errors->has('districts') ? 'is-invalid' : '' }}" type="text" name="districts" id="districts" value="{{ old('districts', '') }}" required>
            ...
            <div class="form-group col-lg-4">
                ...
                <select class="form-control select2 {{ $errors->has('select_state') ? 'is-invalid' : '' }}" name="select_state_id" id="select_state_id" required>
                    @foreach($select_states as $id => $entry)
                        <option value="{{ $id }}" {{ old('select_state_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
