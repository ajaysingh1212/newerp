...
<div class="max-w-6xl mx-auto py-8">
    <div class="bg-white shadow-xl rounded-2xl p-8 border border-gray-200">
        ...
        <div class="pb-4 border-b mb-6 flex items-center justify-between">
        ...
        <div id="top-popup" class="fixed left-1/2 transform -translate-x-1/2 top-10 z-50 hidden">
            <div id="top-popup-body" class="rounded-lg px-5 py-3 text-sm font-medium shadow-lg"></div>
        </div>

        <form method="POST" action="{{ route('admin.investments.update', [$investment->id]) }}" enctype="multipart/form-data" id="investment-form">
            @method('PUT')
            @csrf
            ...
            <div class="grid grid-cols-1 gap-6">
                ...
                <div class="flex flex-col md:flex-row gap-6">
                        ...
                        @if($userRole === 'Admin')
                            <select name="select_investor_id" id="select_investor_id" class="w-full rounded-lg border-2 border-gray-200 px-4 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-200">
                                ...
                                @foreach($select_investors as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('select_investor_id', $investment->select_investor->id ?? '') == $id) ? 'selected' : '' }}>
                                        {{ $entry }}
                                    </option>
                                @endforeach
                            </select>
                        @else
                            @if(isset($investment->select_investor))
                                <p class="text-sm text-gray-700">Investor: <strong>{{ $investment->select_investor->reg }}</strong></p>
                                <input type="hidden" name="select_investor_id" value="{{ $investment->select_investor->id }}">
                            @else
                                <p class="text-sm text-red-600">Investor record not available.</p>
                            @endif
                        @endif

                        <div id="investor-card" class="mt-6 rounded-xl overflow-hidden border shadow-sm relative bg-white hidden">
                            <div class="p-6">
                                <div class="flex items-start justify-between">
                                    <div>
                                        <h3 id="investor-reg" class="text-2xl font-semibold text-gray-800"></h3>
                                    </div>
                    ...
                    <div class="w-full md:w-1/2">
                        ...
                        <div class="plan-area bg-white border rounded-2xl p-4 shadow-sm">
                                ...
                                @foreach($plans as $plan)
                                    <div class="plan-card relative border rounded-lg p-4 shadow-sm" data-plan='@json($plan)'>
                                        <div class="flex items-start justify-between">
                                            ...
                                            <div>
                                                <input type="checkbox" name="select_plan_id" value="{{ $plan->id }}" class="plan-checkbox h-5 w-5" {{ (old('select_plan_id', $investment->select_plan->id ?? '') == $plan->id) ? 'checked' : '' }}>
                                            </div>
                                        </div>
                            ...
                            @error('select_plan_id') <p class="text-sm text-red-600 mt-2">{{ $message }}</p> @enderror
                        </div>
                    </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div class="bg-green-50 p-4 rounded-lg">
                        <label for="principal_amount" class="block text-sm font-semibold text-gray-700 mb-1 required">{{ trans('cruds.investment.fields.principal_amount') }}</label>
                        <input type="number" step="0.01" name="principal_amount" id="principal_amount" value="{{ old('principal_amount', $investment->principal_amount) }}" class="w-full rounded-lg border-gray-300 shadow-sm px-3 py-2 text-sm" required readonly>
                        @error('principal_amount') <p class="mt-2 text-sm text-red-600">{{ $message }}</p> @enderror
                    </div>

                    <div class="bg-yellow-50 p-4 rounded-lg">
                        <label for="secure_interest_percent" class="block text-sm font-semibold text-gray-700 mb-1">{{ trans('cruds.investment.fields.secure_interest_percent') }}</label>
                        <input type="text" name="secure_interest_percent" id="secure_interest_percent" value="{{ old('secure_interest_percent', $investment->secure_interest_percent) }}" class="w-full rounded-lg border-gray-300 shadow-sm px-3 py-2 text-sm" readonly>
                        @error('secure_interest_percent') <p class="mt-2 text-sm text-red-600">{{ $message }}</p> @enderror
                    </div>

                    <div class="bg-yellow-50 p-4 rounded-lg">
                        <label for="market_interest_percent" class="block text-sm font-semibold text-gray-700 mb-1">{{ trans('cruds.investment.fields.market_interest_percent') }}</label>
                        <input type="text" name="market_interest_percent" id="market_interest_percent" value="{{ old('market_interest_percent', $investment->market_interest_percent) }}" class="w-full rounded-lg border-gray-300 shadow-sm px-3 py-2 text-sm" readonly>
                        @error('market_interest_percent') <p class="mt-2 text-sm text-red-600">{{ $message }}</p> @enderror
                    </div>

                    <div class="bg-yellow-50 p-4 rounded-lg">
                        <label for="total_interest_percent" class="block text-sm font-semibold text-gray-700 mb-1">{{ trans('cruds.investment.fields.total_interest_percent') }}</label>
                        <input type="text" name="total_interest_percent" id="total_interest_percent" value="{{ old('total_interest_percent', $investment->total_interest_percent) }}" class="w-full rounded-lg border-gray-300 shadow-sm px-3 py-2 text-sm" readonly>
                        @error('total_interest_percent') <p class="mt-2 text-sm text-red-600">{{ $message }}</p> @enderror
                    </div>

                    <div class="bg-purple-50 p-4 rounded-lg">
                        <label for="start_date" class="block text-sm font-semibold text-gray-700 mb-1 required">{{ trans('cruds.investment.fields.start_date') }}</label>
                        <input type="text" name="start_date" id="start_date" value="{{ old('start_date', $investment->start_date ? \Carbon\Carbon::parse($investment->start_date)->format('d-m-Y') : \Carbon\Carbon::now()->format('d-m-Y')) }}" class="w-full rounded-lg border-gray-300 shadow-sm px-3 py-2 text-sm" readonly>
                ...
                @if($userRole === 'Admin')
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mt-2">
                        ...
                        <div class="p-4 rounded-lg">
                            <label for="status" class="block text-sm font-semibold text-gray-700 mb-1">{{ trans('cruds.investment.fields.status') }}</label>
                            <select name="status" id="status" class="w-full rounded-lg border-gray-300 shadow-sm px-3 py-2 text-sm">
            ...
            <div class="flex justify-end gap-3 pt-6 border-t mt-6">
                <a href="{{ route('admin.investments.index') }}" class="px-4 py-2 bg-gray-100 text-gray-700 rounded-lg shadow-sm hover:bg-gray-200 transition">
                    {{ trans('global.cancel') }}
                </a>

                <button type="submit" class="px-5 py-2 bg-indigo-600 text-white font-medium rounded-lg shadow-md hover:bg-indigo-700 transition">
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        if (selectInvestorEl) {
            ...
            @if(isset($investment->select_investor))
                showInvestorCard(@json($investment->select_investor));
            @endif
        }
...
<style>
    ...
    #top-popup { left: 50%; transform: translateX(-50%); transition: opacity .3s ease, transform .25s ease; }
    #top-popup.show { opacity: 1; transform: translateX(-50%) translateY(6px); }
    #top-popup.hidden { opacity: 0; }
