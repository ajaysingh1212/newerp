...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.vt.title_singular') }}
    </div>

    <div class="card-body">
         @include('watermark')
        <form method="POST" action="{{ route("admin.vts.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="card px-3">
                <h4 class= "text-center mt-2 py-2 bg-1"> Create Vt</h4>
         <div class="row">
            <div class="form-group col-lg-3">
                <label class="required" for="vts_number">{{ trans('cruds.vt.fields.vts_number') }}</label>
                <input class="form-control {{ $errors->has('vts_number') ? 'is-invalid' : '' }}" type="text" name="vts_number" id="vts_number" value="{{ old('vts_number', '') }}" required>
            ...
            <div class="form-group col-lg-3">
                ...
                <input class="form-control {{ $errors->has('sim_number') ? 'is-invalid' : '' }}" type="text" name="sim_number" id="sim_number" value="{{ old('sim_number', '') }}" required>
            ...
            <div class="form-group col-lg-3">
                ...
                <select class="form-control {{ $errors->has('product_status') ? 'is-invalid' : '' }}" name="product_status" id="product_status" required>
                    ...
                    @foreach(App\Models\Vt::PRODUCT_STATUS_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('product_status', 'not_formed') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
