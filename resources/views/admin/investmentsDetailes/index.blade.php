...
<style>
/* ========================================== */
/* FIXED MODAL — Always center on screen      */
/* ========================================== */
.modalOverlay{
...
.planTitle{font-size:18px;font-weight:700;color:#0f172a;}
.planAmount{font-size:15px;font-weight:600;color:#0369a1;margin-top:6px;}
...
<div class="max-w-7xl mx-auto py-8">
    ...
    {{-- ========================================================= --}}
    {{-- ADMIN: DROPDOWN — USER: PLAN CARDS --}}
    {{-- ========================================================= --}}
    <div class="cardBox mb-6" id="select-card">
        ...
        @if($isAdmin)
            ...
            <select id="investment_id" class="w-full rounded-lg border-gray-300 shadow-sm px-3 py-2">
                ...
                @foreach($investments as $inv)
                    <option value="{{ $inv->id }}">
                        #{{ $inv->id }} —
                        {{ optional($inv->select_investor)->reg }}
        ...
        @else
            ...
            {{-- USER — Plan Cards (Multiple Plans) --}}
            @if(count($investments) > 1)
                <h3 class="text-lg font-semibold mb-3">My Investment Plans</h3>

                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4" id="userPlanList">

                    @foreach($investments as $inv)
                        @php
                            $p = $inv->select_plan;
                            $totalPercent = ($p->secure_interest_percent ?? 0) + ($p->market_interest_percent ?? 0);

                            // Dynamic color by percentage
                            if($totalPercent < 10) $clr = "#6b7280";       // Gray
                            elseif($totalPercent < 15) $clr = "#0ea5e9";   // Sky Blue
                            elseif($totalPercent < 20) $clr = "#22c55e";   // Green
                            else $clr = "#f97316";                        // Orange
                        @endphp

                        <div class="planCard"
                             onclick="userSelectPlan({{ $inv->id }})"
                             id="planCard_{{ $inv->id }}">
                            ...
                            {{-- Plan Color Label --}}
                            <div class="planColor" style="background:{{ $clr }}">
                                {{ $p->total_interest_percent ?? 0 }}%
                            </div>
                            ...
                            <div class="planAmount">
                                Principal: ₹{{ number_format($inv->principal_amount,2) }}
                            </div>
    ...
    {{-- If Server Message (No data) --}}
    @if(!empty($message))
        <div class="no-data-card">{{ $message }}</div>
    @endif

    {{-- The result-area (remains EXACT same; shown in PART 2) --}}
    {{-- ========================================================= --}}
    {{-- RESULT AREA — SAME AS YOUR ORIGINAL, NO CHANGE --}}
    {{-- ========================================================= --}}
    <div id="result-area" class="{{ empty($message) ? 'hidden' : 'hidden' }}">
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
            ...
            <div class="cardBox">
                ...
                <div class="mt-4 flex justify-between items-center">
                    <button id="moreDetailsBtn" type="button"
                        class="px-4 py-2 bg-gray-100 text-gray-800 rounded-lg shadow hover:bg-gray-200">
        ...
        <div class="cardBox mt-6">
            ...
            <div class="flex gap-4 items-center">
                <img id="investor_img"
                     class="w-20 h-20 rounded-full ring-2 ring-gray-300"
                     src="/mnt/data/default.png"
                     alt="profile">
        ...
        <div class="cardBox mt-6">
            <div class="flex justify-between">
                ...
                <a href="{{ route('admin.pending.report') }}"
                   class="btn btn-outline-success">
                    📑 Complete Pending / Active Report
                </a>
            </div>
...
{{-- ===================================================================== --}}
{{-- ALL MODALS (YOUR ORIGINAL CODE — NO CHANGE) --}}
{{-- ===================================================================== --}}
    ...
    <div class="modalPanel">
        ...
        <div class="mb-4">
            ...
            <table class="table table-bordered w-full">
                <thead>
                <tr><th>Date</th><th>Daily Interest</th></tr>
                </thead>
...
<script>
document.querySelectorAll(".modalOverlay").forEach(m => m.style.display = "none");
...
document.getElementById("closeApprove")?.addEventListener("click", () =>
    document.getElementById("approveModal").style.display = "none"
);
document.getElementById("ap_cancel")?.addEventListener("click", () =>
    document.getElementById("approveModal").style.display = "none"
);

/* NOTE:
   आपका JavaScript ऊपर Part 1–2 में SAME रहेगा।
   Modals अब हमेशा CENTER में खुलेंगे।
*/
</script>
<script>
/* ============================================================
...
['detailsModal', 'withdrawModal', 'approveModal'].forEach(m => {
    if (el(m)) el(m).style.display = 'none';
});

/* Modal close buttons */
if (el('closeDetails')) el('closeDetails').onclick = () => el('detailsModal').style.display = 'none';
if (el('closeModal')) el('closeModal').onclick = () => el('withdrawModal').style.display = 'none';
if (el('closeApprove')) el('closeApprove').onclick = () => el('approveModal').style.display = 'none';
if (el('ap_cancel')) el('ap_cancel').onclick = () => el('approveModal').style.display = 'none';

/* ============================================================
   INVESTMENTS ARRAY SENT FROM PHP
   ============================================================ */
const INVESTMENTS = {!! json_encode(
    $investments->map(function($i){
        return [
            'id' => $i->id,
            'reg' => optional($i->select_investor)->reg,
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // If there is a server message (no registration or no investments)
    if (serverMessage) {
    ...
    // Admin → show dropdown (already visible)
    if (IS_ADMIN && el('investment_id')) {
        el('investment_id').addEventListener('change', function(){
            if (this.value) loadInvestment(this.value);
        });
        return;
    }
    ...
    if (!IS_ADMIN) {
        ...
        // Hide admin select card
        if (el('select-card')) el('select-card').style.display = 'block';

        // If user has only 1 plan → auto-load
        if (INVESTMENTS.length === 1) {
            const onlyID = INVESTMENTS[0].id;
            setTimeout(() => {
                loadInvestment(onlyID);
            }, 50);
        }
    }
});
...
/* ============================================================
   PLAN CARD CLICK HANDLER — USER ONLY
   ============================================================ */
function userSelectPlan(id) {

    // remove highlight from all cards
    document.querySelectorAll('.planCard')
        .forEach(c => c.classList.remove('selectedCard'));

    // highlight selected card
    const card = el('planCard_' + id);
    if (card) card.classList.add('selectedCard');
...
function renderData(data) {
    ...
    el('principal_plus_interest').innerText =
        '₹' + (
            ...
            Number(data.totalEarnedInterest || 0)
        ).toFixed(2);
    ...
    el('investor_img').src =
        inv.select_investor?.profile_image?.url || '/mnt/data/default.png';
    ...
    (inv.investment_withdrawal_requests || []).forEach(w => {
        ...
        withdrawHTML += `
            <tr>
                ...
                <td>${w.type}</td>
                <td><span class="badge-status ${statusClass}">${w.status}</span></td>
                <td>${w.requested_at || ''}</td>
                <td>
                    <button class="action-btn action-view" onclick="openAdminView(${w.id})">
                        View
                    </button>

                    ${ IS_ADMIN && w.status !== 'approved'
                        ? `<button class="action-btn action-approve"
                                  onclick="openAdminApprove(${w.id})">Approve</button>`
                        : ''
                    }
                </td>
            </tr>
    ...
    const s = data.approvedSummary || {
        total_approved:0, approved_interest:0, approved_principal:0,
        approved_totaltype:0, final_interest:0, final_principal:0,
...
function loadDailyInterestData(id) {
    fetch(FETCH_DAILY_URL(id))
        ...
        .catch(e => console.error('dailyInterest error', e));
}
            ...
            rows.forEach(row => {
                ...
                dailyHTML += `
                    <tr>
                        ...
                        <td>₹${Number(row.daily_interest_amount).toFixed(2)}</td>
                    </tr>
                `;
            });
...
function openWithdrawModal(inv, data) {
    ...
    if (startDate && planLockDays > 0) {
        ...
        const remaining = planLockDays - daysPassed;
        if (remaining > 0) {
            alert("You can withdraw after " + remaining + " more days");
            return;
        }
    }
    ...
    fetch(FETCH_DAILY_URL(id))
        ...
        .then(rows => {
            ...
            const noteEl = el('withdraw_disclaimer');
            ...
            function updateFields() {
                ...
                if (typeEl.value === 'interest') {
                    ...
                    noteEl.innerText = "Interest withdrawals are processed in 24–48 hours.";
                }
                ...
                if (typeEl.value === 'principal') {
                    ...
                    noteEl.innerText = "Principal withdrawals may take 7–14 days.";
                }
                ...
                if (typeEl.value === 'total') {
                    ...
                    noteEl.innerText = "Total withdrawals follow principal withdrawal rules.";
                }
            }

            typeEl.onchange = updateFields;

            amountEl.oninput = () => {
                if (Number(amountEl.value) > Number(amountEl.max)) {
                    amountError.innerText = "Amount exceeds the maximum limit.";
            ...
            el('withdrawForm').onsubmit = e => {
                ...
                if (!el('confirm_checkbox').checked) {
                    alert("Please confirm the withdrawal request");
                    return;
                }
...
function openAdminView(withdrawId) {
    ...
    fetch(WITHDRAWAL_DETAILS_URL(withdrawId))
        ...
        .then(payload => {
            ...
            let info = `Amount: ₹${Number(w.amount).toFixed(2)}\nType: ${w.type}\nStatus: ${w.status}\nRequested At: ${w.requested_at}\n\n`;
    ...
    fetch(WITHDRAWAL_DETAILS_URL(withdrawId))
        ...
        .then(payload => {
            ...
            if (w.approved_at) {
                ...
                el('ap_approved_at').value = new Date(dt - tz).toISOString().slice(0,16);
            } else {
            ...
            if (w.media && w.media.length) {
                el('ap_existing_attachment').innerHTML = w.media.map(m =>
                    `<a href="${m.url}" target="_blank">${m.file_name}</a>`
                ).join('<br>');
            }

            let html = '';
            html += `<div><b>Reg:</b> ${reg.reg || '—'}</div>`;
            html += `<div><b>Aadhaar:</b> ${reg.aadhaar_number || '—'}</div>`;
            html += `<div><b>PAN:</b> ${reg.pan_number || '—'}</div>`;
            html += `<div><b>Bank Holder:</b> ${reg.bank_account_holder_name || '—'}</div>`;
            html += `<div><b>Account:</b> ${reg.bank_account_number || '—'}</div>`;
            html += `<div><b>IFSC:</b> ${reg.ifsc_code || '—'}</div>`;
            html += `<div><b>User Name:</b> ${user.name || '—'}</div>`;

            el('ap_investor_info').innerHTML = html;

            el('approve_errors').style.display = 'none';
            el('approveModal').style.display = 'flex';
        })
        .catch(err => {
            console.error('openAdminApprove error', err);
...
/* ============================================================
   ...
   ============================================================ */
if (el('approveForm')) {
    el('approveForm').onsubmit = function(e) {
        e.preventDefault();
        ...
        if (el('ap_attachment').files[0]) {
            form.append('attachment', el('ap_attachment').files[0]);
        }
        ...
        .then(res => {
            if (res.status === 422)
                return res.json().then(d => Promise.reject({validation:d}));
        ...
        .then(res => {
            alert(res.message || 'Withdrawal updated');
            el('approveModal').style.display='none';

            if (lastLoadedInvestment?.investment?.id) {
        ...
        .catch(err => {
            let box = el('approve_errors');

            if (err.validation) {
                let msgs = Object.values(err.validation.errors).flat().join('<br>');
                box.innerHTML = msgs;
                box.style.display = 'block';
                return;
            }

            box.innerHTML = 'Unable to update. Try again.';
            box.style.display='block';
        });
    };
}
