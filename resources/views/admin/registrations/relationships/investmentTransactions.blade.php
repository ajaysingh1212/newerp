    ...
    <div class="card">
        ...
        <div class="card-body">
            <div class="table-responsive">
                <table class=" table table-bordered table-striped table-hover datatable datatable-investmentTransactions">
                    <thead>
                        <tr>
                            <th width="10">
                            ...
                            <th>
                                {{ trans('cruds.transaction.fields.investor') }}
                            </th>
                            <th>
                            ...
                            <th>
                                {{ trans('cruds.transaction.fields.investment') }}
                            </th>
                            <th>
                    ...
                    <tbody>
                        @foreach($transactions as $key => $transaction)
                            <tr data-entry-id="{{ $transaction->id }}">
                                ...
                                <td>
                                    {{ $transaction->investor->principal_amount ?? '' }}
                                </td>
                                <td>
                                    {{ $transaction->investor->secure_interest_percent ?? '' }}
                                </td>
                                <td>
                                    {{ $transaction->investor->total_interest_percent ?? '' }}
                                </td>
                                <td>
                                    {{ $transaction->investment->reg ?? '' }}
                                </td>
                                <td>
                                    {{ $transaction->investment->aadhaar_number ?? '' }}
                                </td>
                                <td>
                                    {{ $transaction->investment->pan_number ?? '' }}
                                </td>
                                <td>
...
@endcan
  ...
  let table = $('.datatable-investmentTransactions:not(.ajaxTable)').DataTable({ buttons: dtButtons })
