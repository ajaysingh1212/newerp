...
<div class="card me-card animate__animated animate__fadeIn">
    <div class="me-header"><h4 class="mb-0"><i class="fas fa-box mr-2"></i>Edit Product</h4></div>
    <div class="card-body p-4">
        @if($errors->any())
            <div class="alert alert-danger"><ul class="mb-0">@foreach($errors->all() as $e)<li>{{ $e }}</li>@endforeach</ul></div>
        @endif
        <form action="{{ route('admin.manual-products.update', $manualProduct->id) }}" method="POST">
            @csrf @method('PUT')
            <div class="me-form-group mb-3">
                <label class="required">Product Name</label>
                <input type="text" name="name" class="form-control" value="{{ old('name', $manualProduct->name) }}" required>
            </div>
            <div class="me-form-group mb-3">
                <label>Description</label>
                <textarea name="description" rows="4" class="form-control">{{ old('description', $manualProduct->description) }}</textarea>
            </div>
            <button type="submit" class="me-btn-nav"><i class="fas fa-check mr-1"></i> Update Product</button>
