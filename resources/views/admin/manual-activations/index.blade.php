                ...
                <tbody>
                    ...
                    @forelse($activations as $activation)
                        ...
                        <tr
                            ...
                            data-party-id="{{ optional($activation->party)->id ?? '' }}"
                            data-party="{{ e(optional($activation->party)->name ?? '') }}"
                            data-fitter-id="{{ optional($activation->fitter)->id ?? '' }}"
                            data-fitter="{{ e(optional($activation->fitter)->name ?? '') }}"
                            data-product-id="{{ optional($activation->product)->id ?? '' }}"
                            data-product="{{ e(optional($activation->product)->name ?? '') }}"
                            data-state="{{ e($activation->state ?? optional($activation->fitter)->state ?? optional($activation->party)->state ?? '') }}"
                            data-district="{{ e($activation->district ?? optional($activation->fitter)->district ?? optional($activation->party)->district ?? '') }}"
                            data-city="{{ e($activation->city ?? optional($activation->fitter)->city ?? optional($activation->party)->city ?? '') }}"
                            data-fitting-date="{{ e($activation->fitting_date ?? '') }}"
                        >
...
(function () {
    ...
    /* =========================================================
       TOP DASHBOARD FILTERS -> ALL ACTIVATIONS DATATABLE
       The same filters used by the chart are also applied to the
       table below. DataTables will redraw automatically.
    ========================================================= */
    ...
    $.fn.dataTable.ext.search.push(function (
        ...
        if (
            !settings.nTable ||
            settings.nTable.id !== 'allActivationsTable'
        ) {
            return true;
        }
        ...
        const selectedParty =
            String($('select[name="manual_party_id"]').val() || '');

        const selectedFitter =
            String($('select[name="manual_fitter_id"]').val() || '');

        const selectedProduct =
            String($('select[name="manual_product_id"]').val() || '');

        const selectedState =
            String($('#filterState').val() || '').trim().toLowerCase();

        const selectedDistrict =
            String($('#filterDistrict').val() || '').trim().toLowerCase();

        const selectedCity =
            String($('#filterCity').val() || '').trim().toLowerCase();

        const selectedRange =
            String($('#rangeSelect').val() || 'all');

        const fromDate =
            String($('input[name="from_date"]').val() || '');

        const toDate =
            String($('input[name="to_date"]').val() || '');

        const rowPartyId =
            String($row.attr('data-party-id') || '');

        const rowFitterId =
            String($row.attr('data-fitter-id') || '');

        const rowProductId =
            String($row.attr('data-product-id') || '');

        const rowState =
            String($row.attr('data-state') || '')
                .trim()
                .toLowerCase();

        const rowDistrict =
            String($row.attr('data-district') || '')
                .trim()
                .toLowerCase();

        const rowCity =
            String($row.attr('data-city') || '')
                .trim()
                .toLowerCase();

        const rowDate =
            String($row.attr('data-fitting-date') || '').trim();
        ...
        if (
            ...
            // Normalize common Laravel date formats to a JS Date.
            ...
            if (/^\d{4}-\d{2}-\d{2}$/.test(rowDate)) {
                ...
                activationDate = new Date(rowDate);
            }
    ...
    document
        ...
        .addEventListener(
            ...
            function (e) {
                ...
                // Apply the exact same filters to ALL ACTIVATIONS.
                if (activationTable) {
                    activationTable
                        .page('first')
                        .draw();
    ...
    document
        ...
        .addEventListener(
            ...
            function () {
                ...
                // Remove dashboard filters from the table too.
                if (activationTable) {
                    activationTable
                        .search('')
                        .columns()
