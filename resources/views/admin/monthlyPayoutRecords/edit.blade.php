...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.monthlyPayoutRecord.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.monthly-payout-records.update", [$monthlyPayoutRecord->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('investment') ? 'is-invalid' : '' }}" name="investment_id" id="investment_id" required>
                    @foreach($investments as $id => $entry)
                        <option value="{{ $id }}" {{ (old('investment_id') ? old('investment_id') : $monthlyPayoutRecord->investment->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('investor') ? 'is-invalid' : '' }}" name="investor_id" id="investor_id" required>
                    @foreach($investors as $id => $entry)
                        <option value="{{ $id }}" {{ (old('investor_id') ? old('investor_id') : $monthlyPayoutRecord->investor->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('secure_interest_amount') ? 'is-invalid' : '' }}" type="text" name="secure_interest_amount" id="secure_interest_amount" value="{{ old('secure_interest_amount', $monthlyPayoutRecord->secure_interest_amount) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('market_interest_amount') ? 'is-invalid' : '' }}" type="text" name="market_interest_amount" id="market_interest_amount" value="{{ old('market_interest_amount', $monthlyPayoutRecord->market_interest_amount) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('total_payout_amount') ? 'is-invalid' : '' }}" type="number" name="total_payout_amount" id="total_payout_amount" value="{{ old('total_payout_amount', $monthlyPayoutRecord->total_payout_amount) }}" step="0.01" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('month_for') ? 'is-invalid' : '' }}" type="text" name="month_for" id="month_for" value="{{ old('month_for', $monthlyPayoutRecord->month_for) }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('status') ? 'is-invalid' : '' }}" name="status" id="status">
                    ...
                    @foreach(App\Models\MonthlyPayoutRecord::STATUS_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('status', $monthlyPayoutRecord->status) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
