...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.checkPartyStock.title_singular') }}
    </div>

    <div class="card-body">
        @include('watermark')
        <form method="POST" action="{{ route("admin.check-party-stocks.update", [$checkPartyStock->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('select_parties') ? 'is-invalid' : '' }}" name="select_parties[]" id="select_parties" multiple>
                    @foreach($select_parties as $id => $select_party)
                        <option value="{{ $id }}" {{ (in_array($id, old('select_parties', [])) || $checkPartyStock->select_parties->contains($id)) ? 'selected' : '' }}>{{ $select_party }}</option>
                    @endforeach
                </select>
