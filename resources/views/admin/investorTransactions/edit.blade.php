...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.investorTransaction.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.investor-transactions.update", [$investorTransaction->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('investor') ? 'is-invalid' : '' }}" name="investor_id" id="investor_id" required>
                    @foreach($investors as $id => $entry)
                        <option value="{{ $id }}" {{ (old('investor_id') ? old('investor_id') : $investorTransaction->investor->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('investment') ? 'is-invalid' : '' }}" name="investment_id" id="investment_id">
                    @foreach($investments as $id => $entry)
                        <option value="{{ $id }}" {{ (old('investment_id') ? old('investment_id') : $investorTransaction->investment->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('transaction_type') ? 'is-invalid' : '' }}" name="transaction_type" id="transaction_type" required>
                    ...
                    @foreach(App\Models\InvestorTransaction::TRANSACTION_TYPE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('transaction_type', $investorTransaction->transaction_type) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('amount') ? 'is-invalid' : '' }}" type="number" name="amount" id="amount" value="{{ old('amount', $investorTransaction->amount) }}" step="0.01" required>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control ckeditor {{ $errors->has('narration') ? 'is-invalid' : '' }}" name="narration" id="narration">{!! old('narration', $investorTransaction->narration) !!}</textarea>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('status') ? 'is-invalid' : '' }}" name="status" id="status">
                    ...
                    @foreach(App\Models\InvestorTransaction::STATUS_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('status', $investorTransaction->status) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
