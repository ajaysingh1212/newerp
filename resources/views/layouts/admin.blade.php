    ...
    $(document).ready(function () {
        $('[data-widget="pushmenu"]').on('click', function (e) {
            ...
            if (window.innerWidth <= 768) {
              ...
              <!-- Notifications Menu -->
<li class="nav-item dropdown notifications-menu pr-3">
    <a href="#" class="nav-link" data-toggle="dropdown">
        <i class="far fa-bell"></i>
        <span class="badge badge-warning navbar-badge"
              style="{{ auth()->user()->userUserAlerts()->where('read', false)->count() > 0 ? '' : 'display: none;' }}">
            {{ auth()->user()->userUserAlerts()->where('read', false)->count() }}
        </span>
    </a>
        ...
        @if(count($alerts) > 0)
            @foreach($alerts as $alert)
                <div class="dropdown-item">
                    <a href="{{ $alert->alert_link ?? '#' }}" target="_blank" rel="noopener noreferrer">
                        @if(!$alert->pivot->read) <strong> @endif
                            {{ $alert->alert_text }}
                        @if(!$alert->pivot->read) </strong> @endif
                    </a>
                </div>
...
<!-- Notification Script -->
<script>
    let lastAlertCount = {{ auth()->user()->userUserAlerts()->where('read', false)->count() }};
    ...
    function enableAudio() {
        ...
        // This will be triggered by any user interaction
        audioElement.volume = 0.5; // Set volume to 50% to be less intrusive
        audioElement.play().then(() => {
            ...
            console.warn("⚠️ Autoplay still blocked:", err);
        });
    }
    ...
    // Function to fetch alerts and play sound
    function fetchAlerts() {
        fetch("{{ route('admin.alerts.fetch') }}", {
            headers: { 
        ...
        .then(data => {
            const alertsMenu = document.querySelector('.notifications-menu .dropdown-menu');
            const badge = document.querySelector('.notifications-menu .navbar-badge');

            console.log("🔔 Current alerts:", data.count, " | Previous:", lastAlertCount);

            // Play sound only if new alerts arrived and audio is unlocked
            if (data.count > lastAlertCount && canPlaySound) {
            ...
            if (data.html) {
                alertsMenu.innerHTML = data.html;
            }
        })
        .catch(error => {
    ...
    document.addEventListener('DOMContentLoaded', fetchAlerts);
                  ...
                  <!-- Logout -->
                  <li class="pr-3">
                      <a href="#" class="btn btn-outline-danger d-flex align-items-center" onclick="event.preventDefault(); document.getElementById('logoutform').submit();">
                          <i class="fas fa-fw fa-sign-out-alt nav-icon" style="display:flex"></i>
                          {{ trans('global.logout') }}
                      </a>
                  </li>
              </ul>
        ...
        <!-- Dummy top padding to offset fixed navbar -->
    ...
    @include('partials.menu')
        ...
        <div class="content-wrapper" style="min-height: 917px;">
            ...
            <section class="content" style="padding-top: 20px">
                ...
                @endif
                <div class="" style="padding: 20px; background-color: #fff; border-radius: 5px; box-shadow: 0 0 10px rgba(0,0,0,0.1); background: rgba(80, 80, 80, 0.05);">
                     @yield('content')
                </div>
            </section>
            <!-- /.content -->
